...
For any app functionality, see the [Product Requirements Document (PRD)](../docs/app-prd.md).
...
- **Claude** → Symbolic link to this file (`CLAUDE.md → ai/AGENTS.md`)
- **Gemini** → Symbolic link to this file (`GEMINI.md → ai/AGENTS.md`)
- **Codex** → Symbolic link to this file (`AGENTS.md → ai/AGENTS.md`)
- **Windsurf** → Reads `.windsurf/rules/` and writes workflow reports into `/ai/workflows/`
...
- **Redis**: 8.2.x
- **Composer**: 2.8.x
- **Laravel Passport** – OAuth2 authentication for API access  
...
→ See [Creating API Controllers](../docs/requirements/creating-api-controllers.md)
→ Follow [Pagination, Sorting & Filtering](../docs/requirements/pagination-in-controllers.md) when returning list endpoints.
...
→ See [Validation & Requests](../docs/requirements/validation-and-requests.md)

### 4. Resources & Transformation

Use `app/Http/Resources/{ControllerName}/...` for all data formatting.

→ See [Resources & Transformers](../docs/requirements/resources-and-transformers.md)

### 5. Code Style

- Follow PSR-12 and Laravel conventions
- Avoid inline queries — use Eloquent scopes or repositories
...
→ Use `it()` and `test()` functions, not PHPUnit's class-based approach
→ Use `expect()` for assertions when possible
...
- `event-driven-completion-tracking-plan.md` ✅
- `authentication-architecture-decisions.md` ✅
- `simple-todo-list.md` ❌ (use GitHub issue comments)
- `UserController.php` ❌ (use `/app/Http/Controllers`)
...
- You have access to GitHub CLI (`gh`) — use it for all git operations and project updates
...
Never commit `.env` files or secrets; rely on environment-specific configuration and cache settings in production. Rotate API keys through infrastructure tooling and document any new secrets in the internal runbook. Use factories and seeders to provision sample data—avoid hard-coded credentials or identifiers.
