...
class EnsureAuthBridgeRole
{
     ...
     */
    public function handle(Request $request, Closure $next, string $role, ?string $accountId = null, ?string $appKey = null)
    {
        $accountId = $this->resolveIdentifier($accountId, $request, 'account');
        $appKey = $this->resolveIdentifier($appKey, $request, 'app');

        if (! $this->context->hasRole($role, $accountId, $appKey)) {
            throw new HttpException(403, 'Missing required role.');
        }
